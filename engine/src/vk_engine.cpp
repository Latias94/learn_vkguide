...
void VulkanEngine::draw_background(VkCommandBuffer cmd)
{
    ...
    // by it
    vkCmdDispatch(
        cmd, std::ceil(_drawExtent.width / 16.0), std::ceil(_drawExtent.height / 16.0), 1);
}
...
void VulkanEngine::draw_geometry(VkCommandBuffer cmd)
{
    ...
    vkCmdBindDescriptorSets(
        cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, _meshPipelineLayout, 0, 1, &imageSet, 0, nullptr);

    glm::mat4 view = glm::translate(glm::vec3{0, 0, -5});
    // camera projection
    // For the projection matrix, we are doing a trick here. Note that we are sending 10000 to the
    // “near” and 0.1 to the “far”. We will be reversing the depth, so that depth 1 is the near
    // plane, and depth 0 the far plane. This is a technique that greatly increases the quality of
    // depth testing.
    // Depth Reversal, far place has higher precision
    ...
    *sceneUniformData              = sceneData;
    ...
    vkCmdBindIndexBuffer(
        cmd, testMeshes[2]->meshBuffers.indexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);

    vkCmdDrawIndexed(
        cmd, testMeshes[2]->surfaces[0].count, 1, testMeshes[2]->surfaces[0].startIndex, 0, 0);
...
void VulkanEngine::init_background_pipelines()
{
    ...
    VK_CHECK(vkCreateComputePipelines(
        _device, VK_NULL_HANDLE, 1, &computePipelineCreateInfo, nullptr, &gradient.pipeline));
    ...
    VK_CHECK(vkCreateComputePipelines(
        _device, VK_NULL_HANDLE, 1, &computePipelineCreateInfo, nullptr, &sky.pipeline));
...
void VulkanEngine::immediate_submit(std::function<void(VkCommandBuffer cmd)>&& function)
{
    ...
    // One way to improve it would be to run it on a different queue than the graphics queue, and
    // that way we could overlap the execution from this with the main render loop.
...
void VulkanEngine::draw()
{
    ...
    vkutil::transition_image(
        cmd, _drawImage.image, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL);
    ...
    // set swapchain image layout to Present so we can show it
    vkutil::transition_image(cmd,
                             _swapchainImages[swapchainImageIndex],
                             VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
...
void VulkanEngine::run()
{
    ...
    while (!bQuit) {
        ...
        if (ImGui::Begin("background")) {
            ...
            ImGui::SliderInt(
                "Effect Index", &currentBackgroundEffect, 0, backgroundEffects.size() - 1);
                                          ...
                                          bool mipmapped)
{
    ...
    VK_CHECK(vmaCreateImage(
        _allocator, &img_info, &allocinfo, &newImage.image, &newImage.allocation, nullptr));
